#!/usr/bin/env python3
"""
Comprehensive script to fix all router files by adding Request parameter to ALL rate-limited functions.
"""
...
def fix_router_file(file_path):
    ...
    # Fix ALL function definitions that use @limiter.limit
    lines = content.split('\n')
    fixed_lines = []
    i = 0
                        ...
                        if '(' in func_line and ')' in func_line:
                            ...
                            if paren_pos != -1:
                                ...
                                if func_line[paren_pos + 1:].strip().startswith(')'):
                                    ...
                                    func_line = func_line[:paren_pos + 1] + 'request: Request, ' + func_line[paren_pos + 1:]
                        lines[j] = func_line
                        fixed_lines[j - i - 1] = func_line  # Update the line in our output
                    break
                j += 1
